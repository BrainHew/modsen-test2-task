  ...
  return (
    <div className={`flex flex-col md:flex-row p-4 md:p-0 ${isDarkMode ? "bg-gray-800 text-white" : "bg-gray-100 text-gray-800"}`}>
      ...
      <div className="md:w-1/2 md:p-8">
        <h2 className={`text-2xl font-bold mt-4 ${isDarkMode ? "text-white" : "text-gray-800"}`}>{title}</h2>
        <div className={`text-gray-600 mb-4 ${isDarkMode ? "text-white" : ""}`}>By {authors?.join(', ')}</div>
        <div className={`text-gray-600 mb-4 ${isDarkMode ? "text-white" : ""}`}>
          {formattedCategories}
        </div>
